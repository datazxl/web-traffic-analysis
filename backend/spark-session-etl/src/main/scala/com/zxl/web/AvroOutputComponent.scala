package com.zxl.spark.web

import com.zxl.web.AvroRecordBuilder
import com.zxl.web.session.DataRecords
...
trait AvroOutputComponent {
  ...
  private def toRecordsMap(dataRecords: Seq[DataRecords]): Map[String, Seq[GenericRecord]] = {
    ...
    val heartbeats = dataRecords.flatMap(_.heartBeatRecords)
